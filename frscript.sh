...
if ! ls -l $HOME | grep "francinette-image" &> /dev/null; then
		git clone https://github.com/WaRtr0/francinette-image.git $HOME/francinette/francinette-image
fi
...
if ls -l $HOME/francinette/francinette-image | grep "francinette.tar" &> /dev/null; then
		docker load < $HOME/francinette/francinette-image/francinette.tar
fi
